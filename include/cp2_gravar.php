...
$saved = 0;
if (!isset($acao) and isset($dd[0]) and (strlen(trim($dd[0]))>0) and (strlen($tabela) > 0) and (strlen($cp[0][1]) > 0)) 
	{
	...
	//		echo "<BR>".$cp[$p][5].'='.$cp[$p][6].'='.$p."=[".$dd[$p]."]==>".$ok."==";
		...
		if ($ok==1)
		{
		if (isset($dd[0]) and (round($dd[0]) > 0) and (strlen($cp[0][1]) > 0)) 
			{
	//		echo "==gravado";
	...
	//		$dd[1] = NULL;
			...
			$sql = $sql . ') values ('.$sql1.')';
			$sqlc = $sql;
			//echo '<div><font color="white">'.$sqlc.'</div>';
...
function gets_fld()
	{
	global $acao,$dd,$cp,$tabela,$estilo;
	$tt='<table cellpadding="0" cellpadding="2" border="0" class="ed" width="100%" '.$estilo.'>';
	$tt.= '<TR><TH width=5%><TH width="95%">';
	for ($k=0;$k<99;$k++)
		{
		if (isset($cp[$k]))
			{
			$tt.="<TR ".coluna().">";
		    $tt.=''.gets('dd'.$k,$dd[$k],$cp[$k][0],CharE($cp[$k][2]),$cp[$k][3],$cp[$k][4],$cp[$k][5],$cp[$k][6]);
			$tt.="</TR>";
			}
		}
	$tt .= '</table>';
...
function editar()
{
	...
	if (cp2_gravar())
		...
		if (strlen($http_redirect) > 0)
			...
			echo '<form id="fl" name="fl" method="post" ';
