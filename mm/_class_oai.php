...
class oai
	...
	function updatex_article()
		...
		function cab()
			{
				$sx = '<table width="100%">
						...
						<BR>Desenvolvido por Rene Faustino Gabriel Junior &copy; 2010
						<BR>Versão v.0.13.13 
						</table>
				';
				return($sx);
		...
		function proximo_processamento()
			{
		...
		function read_link_fopen($link)
			{
				...
				if (strpos($s,'encoding="UTF-8"') > 0)
					{
						$s = troca($s,'encoding="UTF-8"','encoding="ISO-8859-1"');
						$s = utf8_decode($s);
					}
				fclose($rlt);
				return($s);
			}
		function cache_para_coletar()
			{
		...
		function cache_next_harvesting_journal($id)
			{
				$sql = "select * from brapci_journal where id_jnl > $id and jnl_url_oai <> ''";
				$rlt = db_query($sql);
				if ($line = db_read($rlt))
					{
						return($line['id_jnl']);
		...
		function cache_next_harvesting()
			{
				...
				if ($line = db_read($rlt))
					{
						...
						$utf8 = 0;
						$utf8 = 1;
						if (strpos($s,'encoding="UTF-8"')) { $utf8 = 1; }
						if (utf8_detect($s)) { $utf8 = 1; }
						if ($utf8==1)
							{
								$s = troca($s,'encoding="UTF-8"','encoding="ISO-8859-1"');
								//$s = utf8_decode($s);
