...
$tab_max = 450;
$http_edit = 'rel_metodologia_3.php';
$http_redirect = '';
...
$sql = "
select at_metodo_2, at_tecnica, count(*) as total,
`at_metodo_2` AS mt1,  `at_tecnica` AS mt2,
m1.`bmt_descricao` AS dt1, m2.`bmt_descricao` AS dt2, ar_edition, ar_section
from (
select ar_section, ar_edition,`ar_codigo`,`at_metodo_2`,`at_tecnica_1` as at_tecnica from brapci_article as tb1 where not isnull(at_metodo_2 ) and at_mt = '".$dd[3]."' and ar_status <> 'X' and ar_journal_id = '0000003'
union 
select ar_section, ar_edition,`ar_codigo`,`at_metodo_2`,`at_tecnica_2` as at_tecnica from brapci_article as tb2 where not isnull(at_metodo_2 ) and at_mt = '".$dd[3]."' and ar_status <> 'X' and ar_journal_id = '0000003'
union 
select ar_section, ar_edition,`ar_codigo`,`at_metodo_2`,`at_tecnica_3` as at_tecnica from brapci_article as tb3 where not isnull(at_metodo_2 ) and at_mt = '".$dd[3]."' and ar_status <> 'X' and ar_journal_id = '0000003'
) as tabela
inner join brapci_edition on ar_edition = ed_codigo  
inner join brapci_section on ar_section = se_codigo  
LEFT JOIN brapci_metodologias AS m1 ON at_metodo_2 = m1.`bmt_codigo`
...
$sql .= " group by at_metodo_2, at_tecnica ";
$rlt = db_query($sql);
...
while ($line = db_read($rlt))
	{
	$link = '<A HREF="rel_metodologia_2_detalhes.php?dd3='.$dd[3].'&dd1='.$line['mt1'].'&dd2='.$line['mt2'].'&dd3='.$line['mt3'].'" target="_news" class="lt1">';
	$tp1 = $line['dt1'];
	if ($tp1 != $x)
		{
		if ($to1 > 0)
