...
$sql .= " where ar_status <> 'X' and (ar_resumo_1 like '%&amp;%') ";
...
while ($line = db_read($rlt))
	{
	$link = '<A HREF="http://www.brapci.ufpr.br/mm/brapci_artigos_resumo_pagecode_auto.php?dd0='.$line['id_ar'].'" target="new_">';
	$jnome = $line['jnl_nome'];
	if ($journal != $jnome)
		{
		$s .= '<TR valign="top" >';
...
<BR>total de <B><?=$tot;?> trabalho(s) localizado(s)</B>
</DIV>