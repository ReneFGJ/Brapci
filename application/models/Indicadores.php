...
class indicadores extends CI_Model {
    ...
    function indicador_producao_ano($j = 0) {
        $wh = '';
        if (strlen($j) > 0) {
            if (is_array($j)) {
                $j = strzero($j, 7);
                ...
                $j = strzero($j, 7);
                $wh = " and ar_journal_id = '$j' ";
            }
        }
        $sql = "SELECT * FROM ( 
							SELECT count(*) as total, ed_ano as ar_ano 
							FROM brapci_article 
							...
							WHERE ar_status <> 'X' and jnl_tipo = 'J' and ed_ano > 1950 
							$wh and (se_tipo = 'B')
							GROUP BY ed_ano 
							) as tabela 
						order by ar_ano";
    ...
    function indicador_producao_journals_ano($j='') {
                $wh = '';
        if (strlen($j) > 0)
            {
                if (is_array($j))
    ...
    function indicador_autores_producao($j = '') {
        $wh = '';
        if (strlen($j) > 0) {
            if (is_array($j)) {
                $j = strzero($j, 7);
