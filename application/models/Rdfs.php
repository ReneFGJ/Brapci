...
class rdfs extends CI_model {
	...
	function rdf_article($data) {
		if (!isset($data['ar_codigo'])) { return(''); }
		
		if (strlen($data['ar_codigo']) == 0) {
			return ('');
		}
		...
		/*********** source ***********/
		if (strlen(trim($data['jnl_nome'])) > 0)
			{
				$sx .= '	dc:source "' . $data['jnl_nome'] . '";' . cr();
				$sx .= '	dc:publisher <' . base_url('index.php/rdf/journal/' . $data['jnl_codigo']) . '> ;' . cr();
				
			}
		$sx .= '	bibo:volume "' . $data['ed_vol'] . '";' . cr();
		if (strlen($data['ed_nr']) > 0) { $sx .= '	bibo:number "' . $data['ed_nr'] . '" ;' . cr();
		}
		if (strlen($data['ed_ano']) > 0) {
		...
		if (strlen(trim($data['ar_pg_inicial'])) > 0) {
		...
		$resumo = $data['ar_resumo_1'];
		$resumo = troca($resumo, chr(13), ' ');
		$resumo = troca($resumo, chr(10), ' ');
		$resumo = troca($resumo, '"', '´');
		$resumo1 = $resumo;

		$resumo = $data['ar_resumo_2'];
		$resumo = troca($resumo, chr(13), ' ');
		$resumo = troca($resumo, chr(10), ' ');
		$resumo = troca($resumo, '"', '´');
		$resumo2 = $resumo;
		if (strlen($data['ar_idioma_1']) > 0) {
			$sx .= '	dc:language "' . msg($data['ar_idioma_1']) . '"@' . troca($data['ar_idioma_1'], '_', '-') . ';' . cr();
			if (strlen($resumo1) > 0) {
				$sx .= '	dc:description "' . $resumo1 . '"@' . troca($data['ar_idioma_1'], '_', '-') . ' ;' . cr();
			}
		}
		if (strlen($data['ar_idioma_2']) > 0) {
			$sx .= '	dc:language "' . msg($data['ar_idioma_2']) . '"@' . troca($data['ar_idioma_2'], '_', '-') . ';' . cr();
			if (strlen($resumo2) > 0) {
				$sx .= '	dc:description "' . $resumo2 . '"@' . troca($data['ar_idioma_2'], '_', '-') . ' ;' . cr();
			}
		}
		if (strlen($data['at_metodo_1']) > 0) { $sx .= '	brapci:metodology <' . base_url('index.php/rdf/metodology/' . $data['at_metodo_1']) . '> ;' . cr();
		}
		if (strlen($data['at_metodo_2']) > 0) {	$sx .= '	brapci:metodology <' . base_url('index.php/rdf/metodology/' . $data['at_metodo_2']) . '> ;' . cr();
		}
		if (strlen($data['at_tecnica_1']) > 0) {$sx .= '	brapci:metodology <' . base_url('index.php/rdf/metodology/' . $data['at_tecnica_1']) . '> ;' . cr();
		}
		if (strlen($data['at_tecnica_2']) > 0) {$sx .= '	brapci:metodology <' . base_url('index.php/rdf/metodology/' . $data['at_tecnica_2']) . '> ;' . cr();
		}
		if (strlen($data['at_tecnica_3']) > 0) {$sx .= '	brapci:metodology <' . base_url('index.php/rdf/metodology/' . $data['at_tecnica_3']) . '> ;' . cr();
		}
		if (strlen($data['at_analise_1']) > 0) {$sx .= '	brapci:metodology <' . base_url('index.php/rdf/metodology/' . $data['at_analise_1']) . '> ;' . cr();
		}
		if (strlen($data['at_analise_2']) > 0) {$sx .= '	brapci:metodology <' . base_url('index.php/rdf/metodology/' . $data['at_analise_2']) . '> ;' . cr();
		}
		if (strlen($data['ed_ano']) > 0) {$sx .= '	time:year "' . $data['ed_ano'] . '" ;' . cr();
		}
		$sx .= '	dc:rights "Copyright to the author and publisher" .'.cr();
		$sx .= cr();
		return ($sx);
	}
