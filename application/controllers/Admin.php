...
class admin extends CI_Controller {
    ...
    function tools() {
        ...
        array_push($menu, array(msg('Public Module'), msg('Export File to Public'), 'ITE', '/admin/export_file'));
        array_push($menu, array(msg('Autoridade'), msg('Check remissive authors n use'), 'ITE', '/admin/author_use'));
    ...
    function xls($j = '') {
        if ($j == '') {
            ...
            $sql = "select * from brapc607_public.artigos ";
            $sql .= "where ar_journal_id = '$j' ";
            $rlt = $this -> db -> query($sql);
            ...
            for ($r = 0; $r < count($rlt); $r++) {
                ...
                $s .= '","';
                $s .= $line['Journal_Title'];
                $s .= '","';
                $s .= $line['Author_Analytic'];
                $s .= '","';
                $s .= $line['ar_titulo_1'];
                $s .= '","';
                $s .= $line['ar_ano'];
