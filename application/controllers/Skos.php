...
class skos extends CI_Controller {
	...
	function index() {
		...
		$tela = '<a href="#" onclick="newwin(\'' . base_url('index.php/skos/edc/0/0') . '\',400,400);">';
		$tela .= '<img src="' . base_url('img/icon/icone_mais.png') . '" width="24" align="right" border=0></A>';
		$data['content'] = $tela;
		$this -> load -> view('content', $data);

		$data = array();
		$data['content'] = $this -> skoses -> resumo();
		$this -> load -> view('content', $data);
		...
		if (strlen($term) >= 3) {
			...
			$tela .= $this -> skoses -> search_term($term);
			$tela .= '</td></tr></table>';
			$data['content'] = $tela;
			$this -> load -> view('content', $data);
		} else {
			$letter = get("dd0");
			$data['content'] = $this -> skoses -> alphabetic_list($letter) . '===' . $letter;
			$this -> load -> view('content', $data);

			if (strlen($letter) > 0) {
				$data['content'] = $this -> skoses -> show_terms_by_letter($letter);
				$this -> load -> view('content', $data);
			}
		}
	...
	function t($term = '') {
		...
		$data = $this -> skoses -> le_c($term);
		$this -> load -> view('wese/view', $data);
	}
