...
class article {
	...
	function actions() {
		...
		switch ($sta) {
			case 'A' :
				array_push($act, array('B', 'Enviar para Revis�o 1'));
				array_push($act, array('C', 'Enviar para Revis�o 2'));
				array_push($act, array('D', 'Finalizar revis�o'));
		}
	...
	function verifica_artigo_sem_edicao() {
		...
		while ($line = db_read($rlt)) {
			$sql = "update brapci_article set ar_status = 'X' ,
							ar_resumo_3 = '**Cancelado automaticamente por falta da edi��o **** " . date("d/m/Y H:i:s") . "'
	...
	function cp_issue() {
		...
		if ($line = db_read($rlt)) {
			...
			array_push($cp, array('$O ' . $op, 'ar_edition', 'Edi��o', False, True, ''));
			return ($cp);
		} else {
	...
	function cp_title() {
		...
		array_push($cp, array('$T60:3', 'ar_titulo_1', 'T�tulo principal', True, True, ''));
		array_push($cp, array('$Q ido_descricao:ido_codigo:select * from ajax_idioma order by ido_ordem, ido_descricao', 'ar_idioma_1', 'Idioma', False, True, ''));
		array_push($cp, array('$T60:3', 'ar_titulo_2', 'T�tulo 2� idioma', False, True, ''));
		array_push($cp, array('$Q ido_descricao:ido_codigo:select * from ajax_idioma order by ido_ordem, ido_descricao', 'ar_idioma_2', 'Idioma', False, True, ''));
