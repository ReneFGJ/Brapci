...
class cited {
	...
	function cited_III_recover($id = 0) {
		...
		$proc = 0;
		$idx = 0;
		$sx .= '<table border=1 class="tabela00">';
		while ($line = db_read($rlt)) {
			$idx++;
			$id = $line['id_m'];
			$tt = trim($line['m_ref']);
			...
			if (count($art) == 1)
				{
					$sx .= '<TD>'. ' <font color="blue">LOCALIZADO '.$art[0].'</font>';
					$this->identificar_referencia($art[0],$id);
				} else {
					$sx .= '<TD>'. $this->mostra_botao_editar($id);
					$sx .= '<TD>'. '<font color="red">Localizados '.count($this -> busca_limitador($tt, $ano)).'</font>';
				}
		}
		$sx .= '</table>';
		echo $sx;
	...
	function mostra_botao_editar($id = 0) {
		$link = ' onclick="newxy2(\'article_ref_edit.php?dd0=' . $id . '\',800,300);" ';

		$sx .= '<input type="button" '.$link.' value="Editar Referências">';
