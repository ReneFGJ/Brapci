	...
	function programas()
		{
			...
			$sx .= '<TR><TH>programa<TH>instituição<TH>mestrado<TH>doutorado<TH>mestrado<BR>profissional';
			$sx .= '<TH>professores';
		...
		function mostra_docentes_orientacoes($docente)		
			{
				...
				$sx = '<fieldset>';
				$SX .= '<H2>Orientações Pós-Graduação</h2>';
				...
				$sx .= '   <TH rowspan=2>Programa de <nobr>Pós-Graduação</nobr>';
				$sx .= '   <TH colspan=2>Mestrado';
				$sx .= '   <TH colspan=2>Doutorado';
				$sx .= '   <TH colspan=2>Pós-Doutorado';
				
				$sx .= '   <TH colspan=2>Total de Orientações';
				$sx .= '<TR><TH colspan=1 class="lt0"><center>em orientação';
				$sx .= '    <TH colspan=1 class="lt0"><center>concluídas';
				$sx .= '    <TH colspan=1 class="lt0"><center>em orientação';
				$sx .= '    <TH colspan=1 class="lt0"><center>concluídas';
				$sx .= '    <TH colspan=1 class="lt0"><center>em orientação';
				$sx .= '    <TH colspan=1 class="lt0"><center>concluídas';
				$sx .= '    <TH colspan=1 class="lt0"><center>em orientação';
				...
				//$sx .= '<TR><TD colspan=10 class="lt0">Equivalência de orientações = ';
				//$sx .= '2x(número de doutorados) + (número de mestrados)'; 
				$sx .= '</table>';	
				$sx .= '</fieldset>';	
				return($sx);				
			}
		...
		function mostra_docentes_orientacoes_detalhe($docente,$programa)
			{
				...
				$sx1 .= '<FIELDSET>';
				$sx1 .= '<h2>Relação de Orientandos</h2>';
				...
				if (strlen($po) > 0)
					{ $sx1 .= '<TR><TD colspan=6><B>Pós-Doutorados</B>'.$po;}

				$sx1 .= '</table>';
				$sx1 .= '</fieldset>';
				/* Calculo de equivalencia de orientação */
		...
		function orientacoes($programa='',$professor='',$ano_i=1999,$ano_f=2099)
			{
				...
				$sx .= '<TR><TD colspan=10 class="lt3"><center>Orientações em andamento';
				$sx .= '<TR><TH width="40%">Orientador<TH width="40%">Discente<TH width="7%">Entrada<TH width="7%">Saída<TH width="7%">Modalidade';
				$sx .= $scur;
				$sx .= '</table>';
				...
				$sx .= '<table width="100%" class="lt0">';
				$sx .= '<TR><TD colspan=10 class="lt3"><center>Orientações concluídas';
				$sx .= '<TR><TH width="40%">Orientador<TH width="40%">Discente<TH width="7%">Entrada<TH width="7%">Saída<TH width="7%">Modalidade';
				$sx .= $stit;
				$sx .= '</table>';
...
//				array_push($cp,array('$QA pp_nome:pp_cracha:select * from docentes where pp_ss = \'S\' and pp_ativo = 1 order by pp_nome','pdce_docente',msg('docente'),True,true));
				...
				array_push($cp,array('$O 1:SIM&0:NÃO&-1:Cancelado','pdce_ativo',msg('ativo'),true,true));
		...
		function cp()
			{
				...
				array_push($cp,array('$O 1:SIM&0:NÃO','pos_ativo',msg('ativo'),true,true));
				...
				array_push($cp,array('$O 0:NÃO'.$opa,'pos_mestado',msg('mestrado'),true,true));
				array_push($cp,array('$O 0:NÃO'.$opa,'pos_mestrado_prof',msg('mestrado_profissional'),true,true));
				array_push($cp,array('$O 0:NÃO'.$opa,'pos_doutorado',msg('doutorado'),true,true));
				
				array_push($cp,array('$O 0:NÃO&1:SIM','pos_corrente',msg('corrente'),true,true));
		...
		function mostra_docente_programa($docente)
			...
			$sx .= '<TR>';
			$sx .= '<TH width="5%">Instituição';
			$sx .= '<TH width="40%">Programa de Pós-Graduação';
			$sx .= '<TH width="40%">Linha de Pesquisa';
			$sx .= '<TH width="80%">Participação';
			$sx .= '<TH width="10%">Entrada';
			$sx .= '<TH width="10%">Saída';
