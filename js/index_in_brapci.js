...
(function MendeleyBookmarklet() {
	if (!window.mendeleybookmarklet)
		window.mendeleybookmarklet = {};
	...
	function createCloseButtonDiv() {
		...
		closeButton.id = "mendeley-bookmarklet-close";
	...
	function getForm(hroot) {
		var div = document.createElement("DIV"), form = document.createElement("FORM"), endpointUrl = hroot + "/import/";
		data = {
			version : 1.1
		};
		if ( typeof MendeleyImporterApi !== "undefined") {
			...
			if (dataCallback && typeof dataCallback === "function")
				data.documents = dataCallback(isBookmarklet);
			if (identityCallback && typeof identityCallback === "function")
				data.identity = identityCallback(isBookmarklet);
	...
	function appendSerialized(form, data, path) {
		...
		for (prop in data) {
			...
			if ( typeof item === "string" || typeof item === "number") {
				...
				appendSerialized(form, item, path ? path + "[" + prop + "]" : prop)
		}
	}
